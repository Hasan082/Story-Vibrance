...
import com.hasan.storyvibrance.Model.PostSaveUserIdFireBase;
...
public class SavedPostHandler {
    ...
    public void handlePostSaved(PostModel post, FirebaseUser user, ImageView savedIcon) {
        ...
        if (!isPostSaved) {
            ...
            PostSaveUserIdFireBase postSaveUserIdFireBase = new PostSaveUserIdFireBase(currentUser);
            post.addSaved(postSaveUserIdFireBase);
        } else {
        ...
        updates2.put("postId", post.getPostId());
//        please sent it to data base here post.getPostId()
